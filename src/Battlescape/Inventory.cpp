 ...
 */
Inventory::Inventory(Game *game, int width, int height, int x, int y, bool base)
...
void Inventory::drawGridHighlights() {
    ...
    const auto doItemHighlight = [&](const BattleItem *const item, const int color) {
        ...
        case INV_GROUND: {
            ...
            Log(LOG_VERBOSE) << "(Inv) rect[" << x << "," << y << "," << x + w << "," << y + h << "]";
...
bool Inventory::fitItem(RuleInventory *newSlot, BattleItem *item, std::string &warning) {
    ...
    for (const auto &slotInfo : *newSlot->getSlots()) {
        if (slotInfo.x > maxSlotX)
            maxSlotX = slotInfo.x;
        if (slotInfo.y > maxSlotY)
            maxSlotY = slotInfo.y;
    }
...
bool Inventory::canBeStacked(BattleItem *itemA, BattleItem *itemB) {
    ...
    if (itemA->getRules() != itemB->getRules())
        return false;
    ...
    for (int slot = 0; slot < RuleItem::AmmoSlotMax; ++slot) {
        ...
        if (ammoA && ammoB) {
            ...
            if (ammoA->getRules() != ammoB->getRules())
                return false;
            // and the same ammo quantity
            if (ammoA->getAmmoQuantity() != ammoB->getAmmoQuantity())
                return false;
        } else if (ammoA || ammoB) {
            return false;
        }
    ...
    return (
        ...
        itemA->getPainKillerQuantity() == itemB->getPainKillerQuantity() && itemA->getHealQuantity() == itemB->getHealQuantity() && itemA->getStimulantQuantity() == itemB->getStimulantQuantity());
}
